	...
	function highlightKeywords(text: string, keywords: string[]): string {
		...
		for (const kw of sorted) {
			...
			result = result.replace(
				...
				'<mark style="background:rgba(250,204,21,0.45);border-radius:2px;padding:0 1px">$1</mark>'
			);
		}
		return result;
	}
...
{#if source}
	...
	<!-- Panel -->
	<aside
		class="fixed right-0 top-0 z-50 flex h-full w-full max-w-[480px] flex-col shadow-2xl"
